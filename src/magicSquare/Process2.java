...
public class Process2 {
	...
	public static int[][][] firstLoop(int MagicSquare[][][]) {
		...
		while( sum<405 ) {
			count++;
			System.out.println();
			System.out.println("FirstLoop: Loop" + count);
			int sum2 = sum;
			...
			*/
			...
			// (1)決定したそのマス自身の候補をすべて0にする
			// (2)まわりのマスの候補からその数をなくしていく
			MagicSquare = Process2.consideredRefining(MagicSquare);
			System.out.println(" Considered Refining");
			// そのマスの候補が一つに絞られなくても、
			// 大マスやその行, 列にその数の候補がそれしかなかったら確定
			MagicSquare = Process2.alreadyOK(MagicSquare);
			System.out.println(" MagicSquare Definiting");
			// sumを計算
			sum = Process2.sum(MagicSquare);
			if( sum2==sum ) {
				System.out.println(" 今回のループで更新はありませんでした。FirstLoopをBreakします。");
				break;
			}
			System.out.println(" sum= " + sum);
			if( sum==405 ) {
				System.out.println(" 解答に成功しました。FirstLoopをbreakします。");
			}
		}
		return MagicSquare;
	}
				...
				for(int j=0; j<n; j++) {
					for(int k=1; k<n+1; k++) {
						if( MagicSquare[i][j][k]!=0 ) {
							...
							// 成功し解が出たら取り出す
							sum = Process2.sum(VirtualMagicSquare);
							if( sum==405 ) {
								System.out.println();
